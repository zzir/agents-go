...
func (h *ApprovalHandler) Reject(c *gin.Context) {
	...
	h.resolve(c, false, req.Reason)
}

func (h *ApprovalHandler) resolve(c *gin.Context, approve bool, reason string) {
	toolCallID := c.Param("tool_call_id")
	runID, err := h.runner.ResolveApproval(c.Request.Context(), toolCallID, approve, reason, nil)
	if err != nil {
