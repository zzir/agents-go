...
// branch, outside any run — the Context panel's "Compact now". It reuses the
// run path's own construction (same summary-model resolution, same adapter,
// same guards), so the fold is exactly the one the threshold would have fired;
// Force only skips the threshold. Nothing to fold (the kept window already
// covers the history) returns compacted=false and no error.
//
