...
export function applyToolResult(msgs: Msgs, toolCallId: string, output: string, display?: ToolResultDisplay): Msgs | null {
  ...
  // The result REPLACES live progress. Every key below patches only when it
  // has a value: the streamed turn must equal its reload (invariant 16), and
  // buildTimeline sets these only when the stored row carries them.
...
// same shape as buildTimeline's fold of the server's update entry — terminal
// only, and never backwards (invariant 21). Pre-terminal states stay on the
// card's live-status props.
...
// startTaskAttempt re-arms a spawn card for a retry: clearing the terminal
// status (the label and the attempt stay) is what lets applyTaskTerminal
// accept the NEW outcome. Only a run BEYOND the one the card describes is a
// new attempt, so a replayed run.started cannot wipe a real outcome.
...
// retry that has already finished. The one entry point for the live events,
// a REST response in hand, and the reconnect sweep (invariant 21).
