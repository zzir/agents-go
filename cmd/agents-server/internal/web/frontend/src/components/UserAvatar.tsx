...
// UserAvatar: the provider's picture when there is one, otherwise the first
// letter of the name. The picture is cross-origin (CSP admits the provider's
// hosts); no referrer leaks the workbench URL to it.
export function UserAvatar({ user, size = 32 }: { user: Person; size?: number }) {
  ...
  return user.avatar_url
    ? <img className="user-avatar" style={style} src={user.avatar_url} alt="" referrerPolicy="no-referrer" />
