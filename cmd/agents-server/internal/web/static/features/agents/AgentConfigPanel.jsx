...
function AgentForm({ initial, onSave, onCancel, mcpServers }) {
  ...
  return h('div', { className: 'form-box' },
    ...
    mcpServers && mcpServers.length > 0 && h('div', { className: 'FormControl' },
      h('div', { className: 'divider' }),
      h('div', { className: 'form-section-title' }, 'MCP Servers'),
      h('div', { className: 'form-checkbox-group' },
        mcpServers.map(s =>
          h('label', { key: s.id, className: 'form-checkbox' },
            h('input', { type: 'checkbox', checked: selectedMcp.includes(s.id), onChange: () => toggleMcp(s.id) }),
            h('span', null, s.name),
            s.connected && h('span', { className: 'form-status-dot form-status-dot--success', style: { width: 6, height: 6, marginTop: 7, marginLeft: -4 } }),
          ),
        ),
      ),
    ...
    showAdvanced && h('div', { className: 'advanced-section' },
      ...
      h('div', { className: 'FormControl' },
        h('label', { className: 'form-checkbox' },
          ...
          h('span', { className: 'form-checkbox-text' },
            h('span', null, 'Enable Retry'),
            h('span', { className: 'form-checkbox-description' }, 'Automatically retry failed model calls with backoff'),
          ),
        ),
      ),
      form.retry_enabled && h('div', { style: { marginLeft: '24px' } },
      ...
      h('div', { className: 'divider' }),
      fc('Approve Tools (HITL)', h('input', { value: form.approve_tools || '', onChange: e => set('approve_tools', e.target.value), placeholder: '["*"] or ["tool_name1","tool_name2"]', className: 'form-control form-control-mono' }), 'JSON array of tool names requiring human approval before execution. Use ["*"] for all tools.'),
      ...
      h('div', { className: 'form-checkbox-group' },
        h('label', { className: 'form-checkbox' },
          ...
          h('span', { className: 'form-checkbox-text' },
            h('span', null, 'Disable Tool Choice Reset'),
            h('span', { className: 'form-checkbox-description' }, 'Keep tool_choice across turns instead of resetting'),
          ),
        ),
        h('label', { className: 'form-checkbox' },
          ...
          h('span', { className: 'form-checkbox-text' },
            h('span', null, 'Use Previous Response ID'),
            h('span', { className: 'form-checkbox-description' }, 'Chain responses server-side, send only deltas'),
          ),
        ),
      ),
