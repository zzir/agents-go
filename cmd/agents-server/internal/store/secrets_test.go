...
func TestSecretsSealedAtRest(t *testing.T) {
	...
	// A credential inside a JSON blob: the sandbox's SSH password and an MCP
	// server's headers are sealed field by field; the rest stays readable.
	sandboxes := NewSandboxStore(db)
	sb := &Sandbox{Name: "remote", Type: "docker", Config: json.RawMessage(`{"host":"ssh://u@h","ssh_password":"hunter2","image":"i"}`)}
	if err := sandboxes.Create(ctx, sb); err != nil {
		t.Fatal(err)
	}
	raw := rawColumn(t, db, "SELECT config FROM sandboxes WHERE id = ?", sb.ID)
