...
func TestSandboxDeleteIfUnreferenced(t *testing.T) {
	...
	refs, err := sandboxes.DeleteIfUnreferenced(ctx, "sb-1")
	if err != nil || refs != 1 {
		t.Fatalf("referenced delete: refs=%d err=%v, want a refusal naming 1", refs, err)
	}
	if _, err := sandboxes.Get(ctx, "sb-1"); err != nil {
	...
	if _, err := sandboxes.Get(ctx, "sb-1"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("row survived the delete: %v", err)
	}

	if _, err := sandboxes.DeleteIfUnreferenced(ctx, "sb-none"); !errors.Is(err, ErrNotFound) {
...
func TestSandboxUpdateIdentityIfUnreferenced(t *testing.T) {
	...
	refs, err := sandboxes.UpdateIdentityIfUnreferenced(ctx, "sb-1", moved, 1)
	if err != nil || refs != 0 {
	...
	if _, err := sandboxes.UpdateIdentityIfUnreferenced(ctx, "sb-1", staleWriter, 1); !errors.Is(err, ErrRevisionConflict) {
	...
	// to produce.
	cur, err := sandboxes.Get(ctx, "sb-1")
	if err != nil {
		t.Fatal(err)
	}
	if won, err := sessions.BindSandboxIfEmpty(ctx, sess.ID, "sb-1", "/w", cur.Revision); err != nil || !won {
		t.Fatalf("bind: won=%v err=%v", won, err)
	}
	movedAgain := &SandboxConfig{Name: "sb-1", Type: "ssh", Config: []byte(`{"addr":"third-host","user":"u","work_dir":"/srv"}`)}
	refs, err = sandboxes.UpdateIdentityIfUnreferenced(ctx, "sb-1", movedAgain, cur.Revision)
	if err != nil || refs != 1 {
		t.Fatalf("referenced identity update: refs=%d err=%v, want a refusal naming 1", refs, err)
	}
	got, err := sandboxes.Get(ctx, "sb-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := sandboxes.UpdateIdentityIfUnreferenced(ctx, "sb-none", moved, 1); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing config: err=%v, want ErrNotFound", err)
	}
}
...
func TestSandboxRevisionAndGenerationBumps(t *testing.T) {
	...
	if err := sandboxes.Update(ctx, "sb-1", renamed, 1, false); err != nil {
		t.Fatal(err)
	}
	if got, _ = sandboxes.Get(ctx, "sb-1"); got.Revision != 2 || got.RuntimeGen != 1 {
	...
	if err := sandboxes.Update(ctx, "sb-1", up, 2, true); err != nil {
		t.Fatal(err)
	}
	if got, _ = sandboxes.Get(ctx, "sb-1"); got.Revision != 3 || got.RuntimeGen != 2 {
	...
	if err := sandboxes.Update(ctx, "sb-1", stale, 2, true); !errors.Is(err, ErrRevisionConflict) {
		t.Fatalf("stale update: err=%v, want ErrRevisionConflict", err)
	}
	if _, err := sandboxes.Get(ctx, "sb-1"); err != nil {
		t.Fatal(err)
	}
	if got, _ = sandboxes.Get(ctx, "sb-1"); got.Revision != 3 {
	...
	if refs, err := sandboxes.UpdateIdentityIfUnreferenced(ctx, "sb-1", moved, 3); err != nil || refs != 0 {
		t.Fatalf("identity update: refs=%d err=%v", refs, err)
	}
	if got, _ = sandboxes.Get(ctx, "sb-1"); got.Revision != 4 || got.RuntimeGen != 3 {
	...
	if err := sandboxes.Update(ctx, "sb-none", renamed, 1, false); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing config: err=%v, want ErrNotFound", err)
	}
}
