...
background task (`kind: "workflow"`) whose runs are the steps. The refusal
rules and the row shapes are in
...
- `gate: {pass?, fail?}` makes a step's verdict choose the edge: the last
  non-empty line of its output must be `PASS` or `FAIL` (or the words you
  set), or a JSON object with a boolean `passed` / a `verdict`. A gate that
  reports neither fails the execution — a check that forgot to report is a
  broken step, not a coin flip.
- `pause_before` holds the sequence until a person approves the step from the
  conversation that asked; rejecting cancels the execution.
...
The brief (`input`) leads the first step's turn and is what the sequence
works from — the child session cannot see the conversation, so write what a
colleague picking up the job would need. A start nobody's run asked for (yours,
a trigger's) leaves a `workflow_started` note on the conversation; the result
comes back as a task notification, like any task's.

A step may use tools or hand off, but not spawn tasks or start another
workflow. A busy session (a run executing) or one at its background-task cap
...
- `kind: webhook` fires on `POST /hooks/<trigger id>` (outside `/api/v1`, no
  ...
  `X-Timestamp` is UNIX seconds within five minutes of the server's clock;
  `X-Signature-256` is hex HMAC-SHA256 of `timestamp + "." + body` under the
  secret (a `sha256=` prefix is accepted). The body (64 KB at most) is
  appended to the brief as the payload. A bad or stale signature is `401`;
  the same timestamp and body sent again inside the window is a replay,
  `409` — a retrying sender uses a fresh timestamp. Only a delivery that
  fired is held: one refused because the session was busy may be resent as
  it was.
...
- In a conversation, the top bar's **Tasks** lens lists that session's
  ...
  each step's run ended (`completed`, `failed`, `pass`, `fail`), the brief,
  and the child session's transcript. A step waiting on `pause_before` is an
  approval card there and in the conversation.
...
the stored one line by line. Saving under an existing name replaces that
definition. A saved workflow can be started in the same turn
(`spawn_task(workflow=name)` reads the store). A member's save lands a private
workflow; an existing global name stays an admin's to change
([ownership](workbench-auth.md#ownership-and-roles), workbench
