...
- **Refusals.** A `stop_reason: "refusal"` becomes ONE canonical refusal message and nothing else — the refusal text from the response (else `stop_details.explanation`, else a fixed line), with any partially generated `tool_use` blocks dropped so a refused response's actions never execute. `ModelRefusalError` and `model_refusal` error handlers fire exactly as on the OpenAI backend. When streaming, the per-item `output_item.done` events are held until the stop reason is known, so they always match the terminal output; text deltas still stream live.
- **One message per turn.** Consecutive `text` blocks in a response become ONE message item with a part each — the runner reads only a turn's last message, so a block per item would drop all but the last.
...
`ModelRequest` carries everything a turn needs: `SystemInstructions`, `Input`, `Settings`, `Tools`, `OutputSchema`, `Handoffs`, `PreviousResponseID`, `ConversationID` and `Prompt` (the last three are Responses-only; the Anthropic adapter rejects them).

`ModelResponse` returns `Output` (the output items), `Usage`, `ResponseID` (chains calls via `previous_response_id`), and `RequestID` — the provider request identifier read from the transport response headers (OpenAI's `x-request-id`, Anthropic's `request-id`), handy for support and debugging. Both in-repo providers populate `RequestID` automatically; a custom `Model` leaves it empty when its backend supplies no such header.
