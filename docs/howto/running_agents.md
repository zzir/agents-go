...
- **`Model`** — provider, override, run-level settings and the per-call `InputFilter`: [Models](models.md). `InputFilter` edits what one model call sends; it changes nothing a session saves and does not fire on a resumed turn. `ContextBudget.InputFilter()` is the ready-made one: it tells the model how full its window is ([Sessions](sessions.md#telling-the-model-its-budget)).
