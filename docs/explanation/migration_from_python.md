...
| resume a paused run (state as input to `Runner.run` / `Runner.run_streamed`) | `agents.ResumeRunSync(ctx, state, opts)` / `agents.ResumeRun(ctx, state, opts)`; `agents.ResumeRunWith(ctx, state, opts, ctrl)` resumes on a `RunControl` you already hold |
...
**Structs where Python has unions.** `Tool` and `RunItem` are plain structs — a capability is a field, a run item's variant is its `Kind` ([spec §2.1b](../reference/spec.md#21b-items), [decisions §5.4](decisions.md#54-a-tool-is-a-struct-not-an-interface)). Only `StreamEvent` is an interface you type-switch on.
...
| Model refusal | raises `ModelRefusalError` (recoverable via `error_handlers`) | same: `*ModelRefusalError` carrying the refusal, recoverable via `RunOptions.Exec.ErrorHandlers.ModelRefusal` |
...
| HITL state | `RunState` JSON (Python format) | `RunState` JSON round-trips **Go↔Go only**, and rebuilding needs an agent-name registry (Go functions don't serialize). The state carries `max_turns` so `ResumeRun` continues under the original budget; resumed `NewItems` carry their replayed input form (`Kind` and `Display()` survive, `Raw` is nil) |
| Input guardrail timing | parallel with the **whole first turn** (model call + tool execution); a tripwire cancels the in-flight model task (not billed, no `on_llm_end`) | Overlapped with the model call only — tools never start before guardrails pass, and a tripwire does not cancel the in-flight call (it completes and is billed before the run aborts). Both entry points behave identically; `Blocking: true` gates instead of racing |
| Streamed text items | `message_output_created` fires once per completed message | same (use raw delta events for token-level UI) |
| Session backends | SQLite / SQLAlchemy / Redis / encrypted / OpenAI Conversations / compaction | `session.NewInMemorySession()` in core; `sessions` module adds SQLite/PostgreSQL via bun; `openai.ConversationsSession` (server-side via the Conversations API); `openai.CompactionSession` (`responses.compact` decorator, attempted once per run vs Python's per turn); implement `session.Storage` for anything else |
| Tracing backend | OpenAI traces dashboard by default | generic tracer → processor → exporter pipeline (`ConsoleExporter`, `CollectingExporter`, or any `FuncExporter` — no HTTP exporter, [tracing](../howto/tracing.md#pipeline)); **not** the OpenAI dashboard wire format. Traces export at start, spans at finish |
| Sensitive trace data | `RunConfig.trace_include_sensitive_data` (env `OPENAI_AGENTS_TRACE_INCLUDE_SENSITIVE_DATA`) gates span content | `RunOptions.Observe.IncludeSensitiveData *bool`, default include; **no env var** — the SDK reads none (spec §2.14, decisions §5.39), so nil means include and opting out is `new(false)`. Gates the generation span's request/response data keys (`model`, `system_instructions`, `input`, `tools`, `model_settings`, `handoffs`, `output_schema`, `output`, …) and the function span's `input`/`output` |
...
| `RunItem` discriminator strings | `message_output_item`, `tool_call_item`, … (`_item` suffix) | `RunItem.Kind` is `message_output`, `tool_call`, … (`agents.ItemMessage`, `ItemToolCall`, …) — Go names, stable within Go↔Go `RunState` round-trips |
...
| Session UI metadata | tool-call items may carry `_agents_tool_title`/`_agents_tool_description` keys (stripped before model calls) | sessions store pure API items; SDK-only metadata lives on `RunItem.Extra` (an `ItemToolCallOutput` item, fed by `ToolResult.Details`), never persisted |
...
| `RunState` schema version | its own state versioning | `RunStateSchemaVersion` is `"1.6"`. Decoding accepts a window rather than strict equality: same major, no newer than that minor, no older than the floor (`1.4`) — so once a bump only ADDS fields, a run paused before an SDK upgrade still resumes after it ([decisions §5.18](decisions.md#518-a-runstate-decodes-across-a-version-window-and-the-window-is-earned)) |
...
- *(not yet)* **Redis / encrypted / SQLAlchemy session backends** — only SQLite & PostgreSQL are provided (`sessions` module); implement `session.Storage` for others. (`OpenAIConversationsSession` and `OpenAIResponsesCompactionSession` **are** ported, as `openai.ConversationsSession` and `openai.CompactionSession`.)
...
- **Self-hosted [sandboxes](../howto/sandbox.md)**: run model-written code where you choose — three backends behind one `Sandbox` interface: local, Docker containers (`sandbox/docker`, on the local daemon or a remote one reached over SSH) and any E2B-compatible service (`sandbox/e2b`) — exposed as tools through `sandbox.CodeTool`, the file tools and `apply_patch`. Each backend hosts persistent shells (`exec_command`'s named sessions) and interactive terminals (`TerminalOpener`). Python has since grown its own sandbox stack (self-hosted `docker` / `unix_local` plus hosted providers as extensions); the Go surface is the smaller one — exec, files, shells — with no provider beyond those three
...
- **Run middleware** (`agents/middleware`): `Loop` re-runs until an evaluator accepts, `Approval` answers approval pauses from a standing policy, `Retry` re-runs a failed run, `Plan` and `Todo` add plan mode and a working todo list. Python has no equivalent
