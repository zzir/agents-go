...
| Sensitive trace data | `RunConfig.trace_include_sensitive_data` (env `OPENAI_AGENTS_TRACE_INCLUDE_SENSITIVE_DATA`) gates span content | `RunOptions.Observe.IncludeSensitiveData *bool`, nil = include, opting out is `new(false)`; **no env var** ([spec §2.14](../reference/spec.md#214-the-sdk-reads-no-environment-variable)). Gates the generation span's request/response keys and the function span's `input`/`output` ([tracing](../howto/tracing.md#sensitive-data)) |
...
| Server-side conversation state | `previous_response_id` / `conversation_id` parameters | `RunOptions.Conversation.UsePreviousResponseID` / `ConversationID` — deltas only, neither combines with a local Session ([Sessions](../howto/sessions.md#openai-conversations-server-side)) |
...
- **Provider-level decorators**: `NewRetryProvider` / `NewFallbackProvider` wrap a `ModelProvider` so every model it produces retries or falls back ([Models](../howto/models.md#retries-fallback-and-multiple-providers))
...
- **Run middleware** (`agents/middleware`): `Loop`, `Approval`, `Retry`, `Plan`, `Todo` ([Running agents](../howto/running_agents.md#middleware))
...
- **Enum parse helpers**: `ParseToolNotFoundBehavior(string)` / `ToolNotFoundBehavior.String()` between configuration strings and SDK enum types
