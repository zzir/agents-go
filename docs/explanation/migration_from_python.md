...
| `SQLiteSession` | `sessions.New` (SQLite via the `sessions` module) |
...
| `RunState` schema version | its own state versioning | | `RunState` schema version | its own state versioning | `RunStateSchemaVersion` is `"1.5"` (bumped for nested-state serialization, guardrail-result carriage, then pending input / disclosed tools / server cursor, then the off-chain-history flag). Decoding accepts a window rather than strict equality: same major, no newer than that minor, no older than `runStateOldestDecodableMinor` — so once a bump only ADDS fields, a run paused before an SDK upgrade still resumes after it. The floor sits at `4`, below the current minor: `"1.3"` was released with two different field shapes and the version string cannot tell them apart, while `1.5` only added a field, so a `1.4` state still decodes |. Decoding accepts a window rather than strict equality: same major, no newer than that minor, no older than `runStateOldestDecodableMinor` — so once a bump only ADDS fields, a run paused before an SDK upgrade still resumes after it. The floor equals the current minor today because `"1.3"` was released with two different field shapes and the version string cannot tell them apart; the window opens at the next additive bump |
...
- **Run middleware** (`agents/middleware`): `Loop` re-runs until an evaluator accepts, `Approval` answers approval pauses from a standing policy, `Retry` re-runs a failed run. Python has no equivalent; it replaced the lifecycle-hook interfaces this SDK used to have
