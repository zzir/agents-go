...
| `SQLiteSession` | `filesession.Store` (JSONL file; a `session.Storage`, wrapped by `session.NewSession`) |
...
- **`filesession.Store`**: zero-dependency JSONL persistence with per-path locking and atomic rewrites
